...
void Framebuffer::AttachNewTexture2D(GLenum attachment, int width, int height, int sizedInternalFormat, int filter, int mipMapLevels) {
	if (this->attachedTextures.size() > 0) {
		GLTexture2D* firstTexture = this->attachedTextures.begin()->second.get();
		if (firstTexture->GetWidth() != width || firstTexture->GetHeight() != height) {
			std::cout << "ERROR::FRAMEBUFFER::ATTACHMENT:: The Texturedimensions " << width << "|" << height << 
	...
	std::unique_ptr<GLTexture2D> texture = std::make_unique<GLTexture2D>(width, height, sizedInternalFormat, mipMapLevels);
